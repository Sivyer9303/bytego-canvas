...
+ [调整] new-api 视频玩法改为用户在设置里选择（文生、图生、多图、首尾帧、参考音频、参考视频），请求按所选玩法校验素材数量并写入对应 `media.role`，不再按参考素材自动猜测。

+ [调整] new-api 视频参考图、参考视频和参考音频会先上传到 `/v1/video/input-assets`，生成请求的 `media.source` 只带 HTTPS 地址，不再把文件塞进请求体。

+ [修复] 视频生成完成后会把成片下载到 IndexedDB 再预览，不再把无法鉴权的远程链接或 JSON 误当视频文件。

+ [调整] 视频时长快捷选项改为每 5 秒一档（5–30 秒），自定义上限 30 秒。

+ [调整] new-api 模式下无脚本的视频生成改走统一入口 `POST /v1/video/generations`；本地开发仍使用 `POST /v1/videos`。

+ [修复] 切换 new-api 账号或无法确认登录身份时，先清掉上一个账号同步来的站内渠道再重新拉取，避免同步失败后继续使用上一个账号的密钥。
+ [调整] 画布顶栏品牌改为 bytego画布，并使用站内 logo。
+ [调整] 登录后若本地已有密钥则刷新页面不再全量同步 Token；配置页渠道栏可手动同步最新密钥与分组。
+ [调整] new-api 同步按令牌分组设定模型用途（图片/视频/文本），不再按模型名猜测；对照表在画布仓库维护。
