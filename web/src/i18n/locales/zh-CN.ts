export default {
    ...
    settingsPanels: {
        ...
        video: { title: "视频设置", generationType: "玩法", modes: { text2video: "文生视频", image2video: "图生视频", multi_image: "多图参考", start_end: "首尾帧", reference_audio: "参考音频", reference_videos: "参考视频" }, modeHints: { text2video: "不发送参考图、视频或音频", image2video: "需要正好 1 张参考图", multi_image: "需要 2–9 张参考图", start_end: "需要正好 2 张参考图，按顺序作为首帧和尾帧", reference_audio: "需要 1–9 张参考图和 1 个参考音频；音频请在画布上连接", reference_videos: "需要 1–3 个参考视频，可附带参考图和音频；视频请在画布上连接" }, quality: "清晰度", size: "尺寸", seconds: "秒数", resolution: "分辨率", ratio: "比例", duration: "时长", smart: "智能", output: "输出", generateAudio: "生成声音", watermark: "添加水印", adaptive: "自适应", sizes: { landscape: "横屏", portrait: "竖屏", square: "方形", widescreen: "宽屏", tall: "长图", auto: "自动" }, ratios: { landscape: "横屏", portrait: "竖屏", square: "方形", standardLandscape: "标准横屏", standardPortrait: "标准竖屏", cinematic: "宽银幕", adaptive: "自适应" } },
    ...
    canvas: {
        ...
        export: { defaultProjectName: "bytego画布", defaultNodesName: "画布元素", item: "元素" },
        createMenu: {
    ...
    home: {
        promptError: "获取提示词失败",
        description: "在 <canvas>bytego画布</canvas> 中生成、连接和重组 <content>图片、文字与图形</content>，让创作从单次生成变成连续推演。",
        start: "开始使用",
        openCanvas: "打开画布",
    ...
    config: {
        ...
        migration: {
            title: "全量数据迁移",
            description: "把浏览器里的画布、素材、生成记录和插件数据打包成一个 zip 文件，用于换设备或换浏览器时整体搬迁。API 密钥不会导出。",
            export: "导出全部数据",
            import: "导入全部数据",
            estimate: "预计约 {{size}}",
            estimateHint: "实际文件体积以打包结果为准",
            exportSuccess: "已导出 {{count}} 条记录",
            exportFailed: "导出失败",
            importSuccess: "已导入 {{count}} 条记录，请刷新页面",
            importFailed: "导入失败",
            confirmTitle: "导入会覆盖当前全部本地数据",
            confirmDescription: "当前浏览器中的画布、素材和生成记录会被备份文件完全替换，且无法撤销。建议先导出一份当前数据。",
            confirmOk: "确认覆盖导入",
            reload: "刷新页面",
            stageScan: "枚举本地数据",
            stagePack: "打包 {{store}}",
            ...
            stageWrite: "写入 {{store}}",
            stageImported: "导入完成",
            missingManifest: "备份文件损坏：缺少 manifest.json",
            brokenManifest: "备份文件损坏：manifest.json 无法解析",
            notBackup: "这不是 Infinite Canvas 的备份文件",
            versionMismatch: "备份格式版本 {{fileVersion}} 与当前版本 {{appVersion}} 不一致，请使用同版本导出的备份",
            missingStores: "备份文件损坏：缺少存储清单",
        },
        promptSources: {
            add: "新增来源",
            deleteTitle: "删除「{{name}}」？",
