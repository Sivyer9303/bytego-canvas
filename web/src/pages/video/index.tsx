...
import { VideoSettingsPanel, normalizeVideoResolutionValue, normalizeVideoSizeValue, videoGenerationTypeLabel, videoSizeLabel } from "@/components/video-settings-panel";
...
type GenerationLogConfig = Pick<AiConfig, "model" | "videoModel" | "size" | "vquality" | "videoSeconds" | "videoGenerationType" | "videoGenerateAudio" | "videoWatermark">;
...
export default function VideoPage() {
        ...
        try {
            for (let attempt = 0; attempt < 120; attempt += 1) {
                ...
                if (state.status === "completed") {
                    const stored = await storeGeneratedVideo(state.result, taskConfig);
    ...
    const previewGenerationLog = (log: GenerationLog) => {
        ...
        if (log.config.videoGenerationType) updateConfig("videoGenerationType", log.config.videoGenerationType);
                        ...
                        <div className="mt-6 space-y-5">
                            ...
                            <div className="flex items-center justify-between rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-sm dark:border-stone-800 dark:bg-stone-900 sm:hidden">
                                <span className="truncate text-stone-500 dark:text-stone-400">
                                    {modelOptionLabel(effectiveConfig, model)} · {videoGenerationTypeLabel(effectiveConfig.videoGenerationType)} · {normalizeResolution(effectiveConfig.vquality)}p · {videoSizeLabel(effectiveConfig.size)} · {normalizeVideoSeconds(effectiveConfig.videoSeconds)}s
                                </span>
...
function normalizeLogConfig(log: Partial<GenerationLog>): GenerationLogConfig {
    return {
        ...
        videoGenerationType: log.config?.videoGenerationType || "text2video",
...
function buildLog({ prompt, model, config, references, durationMs, status, task, video, error }: { prompt: string; model: string; config: AiConfig; references: ReferenceImage[]; durationMs: number; status: GenerationLog["status"]; task?: VideoGenerationTask; video?: GeneratedVideo; error?: string }): GenerationLog {
    const logConfig = {
        ...
        videoGenerationType: config.videoGenerationType,
